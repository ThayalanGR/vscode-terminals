...
async function runTerminals(rootPath?: string, substitutions?) {
    const config = await Config.get(rootPath),
        terminals = config.terminals.filter(
            (terminal) => terminal.onlyAPI !== true && terminal.onlySingle !== true,
        );

    if (!terminals.length)
        vscode.window.showErrorMessage('No terminals defined, edit the configuration');

    substitutions = substitutions || Substitutions.get();

    const terms = (
            await Promise.all(
                terminals.map((terminal) => run(terminal, config, rootPath, substitutions)),
            )
        ).filter(_.identity),
        term = terms.find(
            ({ __terminal }) => __terminal.open || __terminal.focus,
        ) as vscode.Terminal;
...
async function runTerminal() {
    const config = await Config.get(),
        terminals = config.terminals.filter(
            (terminal) => terminal.onlyAPI !== true && terminal.onlyMultiple !== true,
        );

    if (!terminals.length)
        return vscode.window.showErrorMessage('No terminals defined, edit the configuration');
...
async function initConfig() {
    ...
    const defaultConfig = {
        ...
        terminals: [
            ...
            {
                name: 'Multi',
                commands: ['echo "Did you know?"', 'echo "You can execute multiple commands"'],
            },
            {
            ...
            },
            {
                ...
                command:
                    'echo "I\'m persistent! Try to reload the window and re-execute this command"',
            },
            {
...
async function editConfig() {
    ...
    if (!rootPath)
        return vscode.window.showErrorMessage(
            'You have to open a project before being able to edit its configuration',
        );
